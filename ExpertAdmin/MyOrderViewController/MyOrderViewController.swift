...
class MyOrderViewController: UIViewController {
    ...
    var ordersSnapshot: [QueryDocumentSnapshot] = []
    ...
    func fetchOrders(with status: Int) {
        ...
        // Создание слушателя только для заказов
                    ...
                    .addSnapshotListener { (orderSnapshot, orderError) in
                        if let orderError = orderError {
                            ...
                            for orderDocument in orderSnapshot!.documents {
                                ...
                                self.createOrderBox(with: self.orders)
                            }
                        }
                    }
            }
    ...
    func createOrderBox(with orders: [(id: String, userName: String,expertName: String, date: String, typeExpertiz: Int, pay: Int, status: Int, userText: String?, price: Int, photo1: String?, photo2: String?, photo3: String?, photo4: String?)]) {
    ...
    @objc func detailsTapped(sender: UIButton) {
        if let orderId = sender.accessibilityIdentifier {
            ...
            detailsVC.orders = self.orders // Предполагается, что у вас есть доступ к массиву заказов
