...
class DetailsOrderViewController: UIViewController {
    ...
    func fetchOrders() {
        ...
        let query = "SELECT id, date, typeExpertiz, UserText FROM orders WHERE typeExpertiz = ? AND result = 1"
    ...
    @objc func MoreDetailsTapped(_ sender: UIButton) {
        ...
        if let id = MoreDetailID.share.id {
            print("ID Выбранной экспертизы: \(id)")
        } else {
            print("ID экспертизы не найден.")
        }
